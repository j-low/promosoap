...
func (c *Client) Call(ctx context.Context, soapAction string, request, response interface{}) (*http.Response, error) {
  ...
  decoder.DefaultSpace = NamespacePDS
  ...
  if err := decoder.Decode(respEnvelope); err != nil {
      return httpResponse, fmt.Errorf("soap/client.go Call(): could not unmarshal response: %w", err)
  }
...
func replaceSoap12to11(data []byte) []byte {
  return bytes.ReplaceAll(data, []byte(NamespaceSoap12), []byte(NamespaceSoap11))
}

func replaceSoap11to12(data []byte) []byte {
  return bytes.ReplaceAll(data, []byte(NamespaceSoap11), []byte(NamespaceSoap12))
}
